...
function App() {
    ...
    const addBookHandler = (name, author, rate, date, notes) => {
        const temp = {title: name, author: author, rate: rate, date: date, notes: notes};
        setBookSet((prev) => {
    ...
    return (
        <div className="App">
            <Navbar deleteAllBooks={deleteAllBooksHandler}
                    showAddBookForm={addBookForm}/>
            <Overlay showOverlay={showAddBook}/>
            {showAddBook ? <AddBook onAddBook={addBookHandler} closeAddBook={addBookFormHide}/>
                : null}
